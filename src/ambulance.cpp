...
void Ambulance::sendPatient(){
    const int maxTry = 20; // nombre d'essais max pour livrer un patient
    int supplierCost = getEmployeeSalary(getEmployeeThatProduces(ItemType::PatientSick));

    this->mutex.lock();
    for(int nbTry = 0; nbTry < maxTry; nbTry++){
        int cost = this->chooseRandomSeller(hospitals)->send(ItemType::PatientSick, 1, getCostPerUnit(ItemType::PatientSick)); //choisi un hopital aléatoirement, puis lui propose "d'acheter" un patient
...
void Ambulance::run() {
    ...
    while (this->getNumberPatients() > 0 and !this->stopRoutine and !stopProgram) { //Tant que son "stock" de patients n'est pas vide et qu'il continue à faire des tranferts
        sendPatient();
